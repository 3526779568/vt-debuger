...
///
		...
		rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionUniqueKeys(bool& canInsert, const key_type& key)
	{
		...
		{
			if (EASTL_LIKELY(pLowerBound != (node_type*)mAnchor.mpNodeLeft) && pLowerBound->mpNodeParent != 0) // If the tree was empty or if we otherwise need to insert at the very front of the tree...
			{//这里需要判断父节点是否是零
...
namespace eastl
{
	...
	///
	EASTL_API inline rbtree_node_base* RBTreeIncrement(const rbtree_node_base* pNode)
	{
	...
	///
	EASTL_API inline rbtree_node_base* RBTreeDecrement(const rbtree_node_base* pNode)
	{
	...
	///
	EASTL_API inline  size_t RBTreeGetBlackCount(const rbtree_node_base* pNodeTop, const rbtree_node_base* pNodeBottom)
	{
		size_t nCount = 0;
	...
	inline rbtree_node_base* RBTreeRotateLeft(rbtree_node_base* pNode, rbtree_node_base* pNodeRoot)
	{
	...
	/// If you want to understand tree rotation, any book on algorithms will
	/// discussion the topic in good detail.
	inline rbtree_node_base* RBTreeRotateRight(rbtree_node_base* pNode, rbtree_node_base* pNodeRoot)
	{
	...
	///
	EASTL_API  inline void RBTreeInsert(rbtree_node_base* pNode,
		rbtree_node_base* pNodeParent,
		rbtree_node_base* pNodeAnchor,
		RBTreeSide insertionSide)
	{
	...
	///
	EASTL_API inline  void RBTreeErase(rbtree_node_base* pNode, rbtree_node_base* pNodeAnchor)
	{
		rbtree_node_base*& pNodeRootRef = pNodeAnchor->mpNodeParent;
